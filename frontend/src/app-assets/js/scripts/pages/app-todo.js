  ...
  addTodo.on("click", function () {
    ...
    function renderAvatar(src) {
      if (src !== undefined) {
        ...
        return '<div class="avatar-content"><i class="feather icon-user font-medium-4"></i></div>'
      }
    };
    ...
    if (taskTitle.val().length > 0) {
      ...
      todoTaskListWrapper.append(
        ...
        '<i class="feather icon-star"></i>' + '</a>' +
        '<a class="todo-item-delete ml-50">' + '<i class="feather icon-trash-2"></i>' + '</a>' +
        '</div></div></li>');
